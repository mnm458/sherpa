...
func Connect(wsUrl string, apiKey string, secret string) {
	address := wsUrl
...
func onOpen(c *websocket.Conn, apiKey string, secret string) {
	fmt.Println("Opened.")
	sendAuth(c, apiKey, secret)
	sendSubscription(c, "order")
}

func sendAuth(c *websocket.Conn, apiKey string, secret string) {
